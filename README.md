...
용어집 기반 탐지(`detect`)·소리 유사도 탐지(`fuzzy_detect`)·보정(`correct`)과
STT 연동(`pipeline`)까지 구현 완료. 번역 연동은 진행 중이다.

**실측** — 1주차 음성 코퍼스(21문장·화자 3인·용어 23건)를 STT부터 관통시켜
**23건 중 22건 복원(95.7%)**. 남은 1건은 `Vue`(`"뷰"`)로, 위 한계에 적은 짧은 음차다.
...
|---|---|
| STT만 | 8/23 = 34.8% |
| STT + hotwords (용어집을 인식 힌트로 넘김) | 17/23 = 73.9% |
| **STT + 보정** (hotwords 없이) | **22/23 = 95.7%** |

**셋째 줄은 hotwords를 쓰지 않은 값이다.** STT가 깨뜨린 것을 사후에 되돌린 결과라,
보정 단계가 혼자 얼마나 하는지를 보여준다.
