...
> ✅ **3인 녹음 완료. 최종 판정.** 판정 기준(측정 전 확정): hotwords 없음 대비 **+5%p 이상이면 채택**.
...
| **화자** | **3명 (A·B·C)** — 팀원 각자 분담 녹음. 담당 구간은 `audio/manifest.yaml`의 `speaker` 필드 |
| 스크립트 | `experiments/risk1_hotwords.py` |
| 원시 로그 | §5 (스크립트 콘솔 출력 원문) |

> 화자는 익명 라벨(A·B·C)로 기록한다. 실험이 증명할 것은 "발음이 다른 3명 모두에서 개선됐다"이지
> 누가 누구인지가 아니다. 화자별 인식률은 스크립트가 자동 집계한다(§2-1).
...
판정 기준 +5%p를 크게 상회한다. **hotwords로 용어를 선별해 STT에 넘기는 모듈을 만들 예정이다**
(토큰 예산 224 안에서 선별).
...
### 2-1. ⭐ 화자별 인식률 — 편향 아님의 근거

`manifest.yaml`의 `speaker` 필드로 스크립트가 자동 집계한다. **"3명이 나눠 녹음했다"를 사람 기억이
아니라 실행 로그로 증명한다.**

| 화자 | hotwords 없음 | hotwords 있음 | 개선폭 |
|------|--------------|--------------|--------|
| A | 2/9 = 22.2% | 6/9 = 66.7% | **+44.4%p** |
| B | 5/7 = 71.4% | 7/7 = 100.0% | **+28.6%p** |
| C | 1/7 = 14.3% | 4/7 = 57.1% | **+42.9%p** |

**세 화자 모두 개선됐다.** 기준선이 14~71%로 제각각인데도 전부 올랐다는 건, hotwords 효과가
특정 화자의 발음에만 맞춰진 게 아니라는 뜻이다. → 예상질문 "편향 없나?"에 대한 정량 답변.
...
인식 뒤에 표준형으로 되돌리는 사후 보정 단계가 왜 필요한가의 근거다.**

| 용어 | 실패 원인 | 어떻게 처리할 것인가 |
|------|-----------|----------------------|
| **Python "파일선"** | 발음이 표준형과 너무 멀어 hotwords 편향이 못 걸림 | 편집거리 기반 퍼지 보정으로 표준형 복원 |
| **React / TypeScript "타입스트리트"** | seed_19 — **한 문장에 용어 2개가 몰리면 약해진다**(§3-4) | hotwords 선별 개선 + 사후 보정 |
| **repository "리포지토리"** | STT는 정확히 발음을 옮겼다. 표준형 영단어만 정답으로 쳐서 MISS | 음차 변형을 표준형에 매칭 |
| **Vue "뷰"** | 짧은 음차. hotwords 있음에서도 "뷰"로 남음 | 음차 매칭 (짧은 별칭은 오탐 주의) |

> `repository`·`Vue`는 STT 실패가 아니다. "리포지토리"·"뷰"로 **정확히 전사됐는데** 우리가
> 표준형만 정답으로 세어서 MISS다. 이건 hotwords의 한계가 아니라 **보정 단계의 몫**이다.
> → 인식(hotwords)과 사후 보정이 왜 둘 다 필요한지를 보여주는 실측 근거.
...
**한 문장에 용어가 몰리면 hotwords 예산이 분산되거나 문맥이 흐려진다**는 신호.
hotwords 용어 선별 전략을 짤 때 고려한다.
...
- [ ] hotwords 용어 선별 모듈 구현 (토큰 예산 224 내 선별 — 조합 문장 §3-4 고려)
- [ ] `data/terms.yaml` alias 갱신 — 실측 음차를 반영:
...
=== 화자별 인식률 (없음 → 있음) ===
  화자 A: 2/9 = 22.2%  →  6/9 = 66.7%  (+44.4%p)
  화자 B: 5/7 = 71.4%  →  7/7 = 100.0%  (+28.6%p)
  화자 C: 1/7 = 14.3%  →  4/7 = 57.1%  (+42.9%p)
  화자 수: 3명
```

차이: +39.1%p / 화자 3명 (A·B·C)
