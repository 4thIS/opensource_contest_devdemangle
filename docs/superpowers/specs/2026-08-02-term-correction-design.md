...
- **조사가 alias 바로 뒤에 붙은 문장은 매칭되지 않는다** (예: `"파일선으로
  작성했어"`, `"기터벳의 리포지토리"`). 시작·끝 모두 토큰 경계와 정확히 일치해야
  하므로, 조사가 없어 alias가 토큰 전체와 같을 때만 매칭된다. `risk1.md`의
  실측 로그 상당수가 이 패턴이라 v1에서는 이런 문장을 못 잡는다. 브레인스토밍
  중 이 문제를 확인했고, 원안(양쪽 경계 고정)을 v1으로 유지하기로 결정했다 —
  안전성(오탐 최소화)을 조사 대응보다 우선. 조사 대응은 다음 이터레이션 대상.
